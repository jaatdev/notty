...
export default function Footer() {
  ...
  const allTags = Array.from(
    new Set(
      subjects.flatMap(s =>
        (s.nodes as any[]).flatMap(n => collectTags(n))
      )
    )
  ).slice(0, 12)
    ...
    <footer className="mt-16 border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-[#0b1220]">
      <div className="max-w-6xl mx-auto px-4 py-10 grid sm:grid-cols-2 md:grid-cols-4 gap-8">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">World‑class, data‑driven notes. Add JSON. Get magic.</p>
          <p className="text-xs text-gray-500 dark:text-gray-500 mt-4">Made with ❤️ for learners.</p>
        </div>
      ...
      <div className="border-t border-gray-200 dark:border-gray-800 text-center py-4 text-xs text-gray-500 dark:text-gray-500">
        © {new Date().getFullYear()} Notty. All rights reserved.
      </div>
    </footer>
  )
}
...
function collectTags(node: any): string[] {
  ...
  if (node?.children?.length) {
    return [...tags, ...node.children.flatMap(collectTags)]
  }
  return tags
}